...
export class RestaurantService {
  ...
  addRestaurant(restaurant: Restaurant, id: string): Observable<Restaurant> {
    const url = `${this.restaurantsUrl}/add/${id}`;
  ...
  deleteMenu(menuId: string): Observable<unknown> {
    const url = `${this.restaurantsUrl}/menus/delete/${menuId}`;
    //console.log(menu);
    return this.http.post(url, this.httpOptions).pipe(
      tap((_) => this.log(`Deleted menu id=${menuId}`)),
